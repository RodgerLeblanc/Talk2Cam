...
{
// This section was modified to use UDP no matter what Talk2Watch version the user use.
// The port 8989 is used for free version as there's a bug in last free version that
// prevents user from changing this value (8989 is default value).
	...
	if(m_talk2WatchProServiceAvailable || m_talk2WatchProAvailable==true)
			...
			m_udp->sendMessage("127.0.0.1", 8989, _command);
